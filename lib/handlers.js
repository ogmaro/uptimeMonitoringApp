                    ...
                    _data.delete("checks", checkid, (err) => {
                      ...
                      if (checksDeleted == checksTODelete) {
                        if (!deletionerror) {
                          ...
                          callback(500, {
                            Error:
                              "Error encountered while attempting to delete all of the user's checks. All checks may not have been deleted from the system successfully",
                          });
                        }
                      }
                    });
...
handlers.tokens = (data, callback) => {
  ...
  if (accepteableMethods.indexOf(data.method) > -1) {
    ...
    callback(405, { msg: "Method not recognized" });
  }
};
        ...
        _data.read("users", userPhone, (err, userData) => {
          if (!err && userData) {
            ...
            if (userChecks.length < config.maxChecks) {
              ...
              callback(400, {
                error: `the user already has the max amount of checks  ${config.maxChecks}`,
              });
            }
          } else {
            callback(403);
  ...
  if (id) {
    _data.read("checks", id, (err, checkData) => {
      if (!err && checkData) {
        ...
        const token =
          typeof data.headers.token == "string" ? data.headers.token : false;
        //Verify that the given token is valid and belongs to the user who create it

        handlers._token.verifyToken(
          token,
          checkData.userPhone,
          (tokenIsValid) => {
            if (tokenIsValid) {
        ...
        callback(404, { Error: "Not found, check doesn't exist" });
      }
    });
  } else {
    callback(400);
...
handlers._checks.put = (data, callback) => {
  ...
  if (id) {
    // checkif an optional field was passed
    if (protocol || url || (method && successCodes) || timeOutSeconds) {
      //Lookup the check
      _data.read("checks", id, (err, checkData) => {
        if (!err && checkData) {
          ...
          callback(400, { Error: `Check Id: ${id} did not exist` });
        }
      });
    } else {
      callback(400, { Error: "Missing an Optional field to update" });
    }
  } else {
    callback(400, { Error: `Id ${id} is or Missing required field` });
  }
};
  ...
  if (id) {
    ...
    _data.read("checks", id, (err, checkData) => {
      if (!err && checkData) {
        ...
        callback(400, { Error: `the specified check id ${id} does not exist` });
      }
    });
  } else {
    callback(404, { error: "cannot find user" });
